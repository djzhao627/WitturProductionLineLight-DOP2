...
public class ComputeSmsData {
    ...
    private String rcvdPduTxt;
     ...
     // *******************************************************************************************************
    //                              encode text message in UCS2 GSM standard
    // *******************************************************************************************************
    private void txtToSmsPdu(String src) {
      byte[] abytes = new byte[src.length()];   
      try  {   
           abytes = src.getBytes("UTF-16BE" );   
      } catch  (UnsupportedEncodingException e) {   
        // throw new PduEncodeException(e);   
      }   
      ...
      for  (int  i = 0 ; i < abytes.length; i++) {   
         ...
         tem.append(Integer.toHexString(abytes[i] & 0xFF ));   
         if (tem.length()== 1 ){   
            tem.insert(0,'0');   
          }   
          reValue.append(tem);   
      }   
      pduTxt =  reValue.toString().toUpperCase();   
    	pduTxtLen = Integer.toHexString((pduTxt.length() / 2) & 0xFF).toUpperCase();
    	if (pduTxtLen.length() == 1){pduTxtLen = "0" + pduTxtLen;}
    }
    ...
    // *******************************************************************************************************
    private void txtToSmsPdu7bit(String testo) {
...
//        System.out.println("Elle2 = " + ll);
    ...
    public String getCompletePduData () {
        ...
        pduData += "81";                // tel # is int'l format
        pduData += telNum;              // tel Num in GSM format
        pduData += "00";                // protocol identifier TP-PID
        pduData += "08";                // pdu data is encoded 7bit data
        pduData += "0A";                // TP validity period
...
//        System.out.println("dec'd Sender info = " + rcvdSenderNum);
    ...
    // ********************************
    // decode pdu text from UCS2 format
    // ********************************    
    
    private String smsPduToTxt(String src) { 
    ...
    // ********************************
    private String smsPduToTxt7bit(String s) {
